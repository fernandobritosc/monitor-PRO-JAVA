...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className="min-h-screen flex bg-[#12151D] text-white transition-all duration-300"> {/* Nova cor de fundo */}
      {sidebarOpen && (
      ...
      <aside className={`
        ...
        ${isCollapsed ? 'lg:w-20' : 'lg:w-64'} {/* Largura da sidebar ajustada */}
        w-72
        bg-[#12151D] z-50 transition-all duration-300 flex flex-col {/* Remove border-r */}
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Fix: Removed comment from inside className string literal */}
        <button 
          ...
          <div className={`flex items-center gap-3 mb-8 shrink-0 transition-all ${isCollapsed ? 'justify-center' : 'px-2'}`}>
            ...
            {!isCollapsed && (
              <div className="animate-in fade-in duration-300 overflow-hidden whitespace-nowrap">
                <h1 className="text-lg font-extrabold tracking-tighter leading-none"> {/* Reduzido de text-xl */}
                  MONITOR<span className="bg-white text-[#12151D] px-1 rounded ml-1">PRO</span> {/* Ajuste de cor de fundo para a nova paleta */}
                </h1>
                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mt-1">Alta Performance</p> {/* Reduzido de text-[10px] */}
              </div>
            )}
          </div>

          <div className={`
            bg-slate-900/30 border border-white/5 rounded-2xl mb-6 shrink-0 relative overflow-hidden group transition-all {/* Ajustado de /50 */}
            ${isCollapsed ? 'p-2 flex justify-center items-center h-14' : 'p-3'} {/* Reduzido de p-4 */}
          `}>
            ...
            {menuItems.map((item: any) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 rounded-xl transition-all duration-200 font-medium text-sm group relative overflow-hidden {/* Removido mb-1 */}
                    ${isCollapsed ? 'justify-center p-3' : 'px-5 py-3'}
                    ${isActive 
                      ? 'bg-purple-500/10 text-white border border-purple-500/20' /* Fundo menos intenso, borda mais sutil */
                      : 'text-slate-400 hover:text-white hover:bg-white/5'
                    }
                  `}
                >
                  <Icon size={isCollapsed ? 20 : 18} className={`${isActive ? 'text-purple-300' /* Ícone mais claro */ : 'text-slate-500 group-hover:text-white'} transition-colors`} />
                  {!isCollapsed && (
      ...
      <main className="flex-1 min-w-0 relative transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto p-4 lg:p-10 pb-24">
          <div className="mb-8">
            <h2 className="text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent uppercase tracking-tight"> {/* Reduzido de text-3xl md:text-4xl */}
              {missaoAtiva || 'Selecione uma Missão'}
            </h2>
            <p className="text-slate-500 font-medium uppercase tracking-widest text-xs mt-1 flex items-center gap-2"> {/* Alterado de text-slate-400 */}
