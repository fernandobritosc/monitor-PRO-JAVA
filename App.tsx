...
const App: React.FC = () => {
  ...
  const handleRecordUpdate = async (updatedRecord: StudyRecord) => {
    ...
    // 3. Se der erro, reverte a UI buscando os dados reais
    if (error) {
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case 'REVISOES': return <Revisoes records={studyRecords} missaoAtiva={missaoAtiva} editais={editais} onRecordUpdate={handleRecordUpdate} onDataRefresh={() => fetchData(session.user.id)} />;
