...
const App: React.FC = () => {
  ...
  // --- GERENCIAMENTO DE SESSÃO ROBUSTO (v1.0.17) ---
  useEffect(() => {
    if (!isConfigured) {
      setNeedsConfig(true);
      setLoading(false);
    ...
    // Timeout de segurança AUMENTADO para 10s
    // Se a conexão estiver lenta, isso evita o logout prematuro
    const safetyTimeout = setTimeout(() => {
        if (mounted && loading) {
            console.warn("Timeout de sessão atingido (10s). Forçando verificação final...");
    ...
    const initializeSession = async () => {
      try {
        // 1. Tenta recuperar sessão padrão
        ...
        if (mounted) {
          if (initialSession) {
            ...
            setLoading(false); 
             ...
             // 2. Fallback: Se não veio sessão, mas tem token no storage, tenta getUser()
             // Isso força o Supabase a validar o token no servidor em vez de confiar apenas no cache local
             const localToken = localStorage.getItem('monitorpro-auth-token');
             if (localToken) {
                 console.log("Token encontrado, mas getSession retornou null. Tentando getUser()...");
                 const { data: { user }, error: userError } = await supabase.auth.getUser();
                 
                 if (user && !userError) {
                     console.log("Usuário validado via getUser. Aguardando sincronização de sessão...");
                     // Não setamos loading=false aqui, deixamos o onAuthStateChange pegar o evento SIGNED_IN
                     // que deve disparar logo em seguida pois o token é válido.
                 } else {
                     console.log("Token inválido ou expirado.");
                     setLoading(false);
                 }
             } else {
                 console.log("Sem token local. Redirecionando para login.");
                 setLoading(false);
             }
          }
        }
        ...
        // Em caso de erro de rede, mantém loading se tiver token, para tentar reconexão
        const hasToken = localStorage.getItem('monitorpro-auth-token');
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, newSession) => {
      ...
      if (mounted) {
        if (newSession) {
             ...
             // Isso impede o "flicker" de login se o navegador demorar pra ler o storage
             const hasLocalToken = localStorage.getItem('monitorpro-auth-token');
             if (!hasLocalToken) {
                setLoading(false);
             } else {
                console.log("Ignorando INITIAL_SESSION null pois existe token local.");
             }
