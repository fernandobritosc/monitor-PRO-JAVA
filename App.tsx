...
// Safe version check
const APP_VERSION = typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : '1.0.21';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleOnline = () => {
        console.log("🌐 Conexão restaurada. Tentando reconectar API...");
        setIsOffline(false);
        if (session?.user?.id) fetchData(session.user.id);
    };

    const handleVisibilityChange = () => {
        if (document.visibilityState === 'visible' && isOffline && session?.user?.id) {
            console.log("👁️ App visível. Tentando reconectar...");
  ...
  // Função de fetch com Retry Logic Agressivo para Mobile
  const fetchData = async (userId: string, retryCount = 0) => {
    // Só mostra loading na primeira tentativa ou se não tiver dados
    ...
    try {
      ...
      // SUCESSO: Limpa estados de erro e loading
      setIsOffline(false);
      ...
      // Retry Logic: Tenta até 4 vezes com backoff (2s, 3s, 4s, 5s)
      if (retryCount < 4) {
          const timeout = (retryCount + 1) * 1500; 
        ...
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl mb-6 flex flex-col md:flex-row items-center gap-4 text-sm font-bold shadow-lg animate-in slide-in-from-top-2">
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                    <span>API Indisponível (Rede Instável).</span>
                    <span className="text-[10px] opacity-70 font-normal">Verifique sua internet. Tentando reconectar...</span>
                </div>
            </div>
            <button 
