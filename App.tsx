...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeSession = async () => {
      // 1. Check Cache FIRST to avoid flicker
    ...
    const { data: { subscription } } = (supabase.auth as any).onAuthStateChange((_event: string, newSession: any) => {
      if (mounted) {
        if (newSession) {
             ...
             // Só resetamos se NÃO for uma renderização inicial onde o cache deve prevalecer.
             // O handleLogout já cuida do logout explícito.
             // Se este evento vier automaticamente (sessão expirada), mantemos o modo offline se disponível.
             setSession(null);
             setUserEmail('');
             if (!loadFromCache()) {
                 setIsOfflineMode(false);
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#12151D] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-600 text-[10px] animate-pulse">Iniciando...</p>
    </div>
  );
