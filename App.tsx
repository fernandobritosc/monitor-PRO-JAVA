...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // onAuthStateChange é a fonte única da verdade para a sessão.
    // Ele dispara imediatamente com a sessão atual (do localStorage) e sempre que ela muda.
    const { data: { subscription } } = (supabase.auth as any).onAuthStateChange((_event: string, session: any) => {
      ...
      // Consideramos o carregamento inicial concluído após a primeira verificação do estado de autenticação.
      setLoading(false);
    });
