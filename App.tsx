...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      setUserEmail(session?.user?.email ?? localStorage.getItem('monitorpro_saved_email') ?? '');
      
      // Se não há sessão e não conseguimos carregar o cache, o hook useAppData vai setar isOfflineMode = false
      if (!session) {
