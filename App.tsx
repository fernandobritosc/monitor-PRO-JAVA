...
const App: React.FC = () => {
  ...
  // --- GERENCIAMENTO DE SESSÃO ROBUSTO (v1.0.19) ---
  useEffect(() => {
    if (!isConfigured) {
      setNeedsConfig(true);
      setLoading(false);
    ...
    const initializeSession = async () => {
      try {
        // Verifica token local antes de chamar API
        const localToken = localStorage.getItem('monitorpro-auth-token');
        
        // 1. Tenta recuperar sessão ativa
        ...
        if (initialSession) {
          ...
          setLoading(false); 
           ...
           // 2. Se tem token mas getSession falhou, tenta um pequeno delay e refresh
           // Isso ajuda em race conditions do navegador/vite
           console.log("🔄 Token detectado, tentando restaurar sessão...");
           ...
           if (refreshedSession && !refreshError) {
               ...
               // Não limpamos imediatamente para dar chance ao onAuthStateChange
           }
           setLoading(false);
        } else {
           console.log("ℹ️ Sem sessão e sem token.");
           setSession(null);
           setLoading(false);
        }
      } catch (error) {
  ...
  // Função de fetch com Retry Logic para Mobile
  const fetchData = async (userId: string, retryCount = 0) => {
    ...
    try {
      ...
      // Retry Logic: Tenta até 2 vezes se falhar (comum em 3G/4G instável)
      if (retryCount < 2) {
          console.log('🔄 Tentando reconectar...');
          setTimeout(() => fetchData(userId, retryCount + 1), 1500); // Espera 1.5s antes de tentar de novo
      } else {
          setIsOffline(true);
      }
    } finally {
