...
const APP_VERSION = '1.0.27'; // Protocolo de Versão Atualizado
...
const App: React.FC = () => {
  ...
  const handleLogout = async () => {
      ...
      // Não limpa o cache de dados para permitir login offline futuro se necessário, 
      // mas limpa a sessão atual.
      setSession(null);
  ...
  useEffect(() => {
    ...
    const initializeSession = async () => {
      // 1. Tenta carregar cache primeiro (para experiência instantânea)
      const hasCache = loadFromCache();
      
      try {
        // 2. Verifica sessão no Supabase
        ...
        if (initialSession) {
          ...
          console.log("✅ Sessão restaurada:", initialSession.user.email);
          setSession(initialSession);
          setUserEmail(initialSession.user.email || '');
          setIsOfflineMode(false); // Estamos online e autenticados
        } else {
           // Sem sessão ativa. Verifica se existe token local e tenta refresh explícito
           const localToken = localStorage.getItem('monitorpro-auth-token');
           if (localToken) {
               console.log("🔄 Tentando refresh de sessão com token existente...");
               const { data: { session: refreshedSession }, error: refreshError } = await (supabase.auth as any).refreshSession();
               
               if (refreshedSession && !refreshError) {
                   console.log("✅ Sessão recuperada via refresh:", refreshedSession.user.email);
                   setSession(refreshedSession);
                   setUserEmail(refreshedSession.user.email || '');
                   setIsOfflineMode(false);
               } else {
                   console.warn("⚠️ Falha no refresh de sessão:", refreshError);
                   // Se falhou o refresh e temos cache, ficamos offline. Se não, login.
                   if (hasCache) setIsOfflineMode(true);
               }
           } else {
               // Sem token e sem sessão -> Modo Offline se tiver cache, senão Login
               if (hasCache) setIsOfflineMode(true);
           }
        }
      } catch (error) {
        console.error("❌ Erro crítico na inicialização da sessão:", error);
        // Em caso de erro, fallback para offline se possível
        if (hasCache) setIsOfflineMode(true);
      } finally {
    ...
    const { data: { subscription } } = (supabase.auth as any).onAuthStateChange((_event: string, newSession: any) => {
      ...
      if (newSession) {
           ...
           setSession(null);
           setUserEmail('');
           // Se saiu, verifica se mantém offline mode ou vai pra login
           // Geralmente logout explícito deve ir pra login
           const hasCache = loadFromCache();
           if (!hasCache) setIsOfflineMode(false);
           setLoading(false);
      }
    });
  ...
  useEffect(() => {
    if (session?.user?.id) {
      fetchData(session.user.id);
       ...
       // Se não tem sessão e não está offline, limpa dados da memória (segurança)
       if (editais.length > 0) {
           setEditais([]);
           setStudyRecords([]);
           setMissaoAtiva('');
  ...
  const fetchData = async (userId: string, retryCount = 0) => {
    // Só mostra loading visual se não tiver dados nenhum
    ...
    try {
      ...
      if (retryCount < 2) {
          ...
          // Se falhar tudo, tenta fallback pro cache se não tiver dados
          if (!hasData) {
              const loaded = loadFromCache();
              if (!loaded) setIsError(true);
          } else {
              // Se já tem dados, assume offline mode temporário
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#12151D] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-600 text-[10px] animate-pulse">Autenticando...</p>
    </div>
  );
