...
const App: React.FC = () => {
  ...
  // const [manualConfigMode, setManualConfigMode] = useState(false); // Removido
  ...
  // Removido: manualConfigMode condition
  // if (manualConfigMode) return <ConfigScreen />; 

  if (loading) return (
    <div className="min-h-screen bg-[#12151D] flex flex-col items-center justify-center gap-4"> {/* Nova cor de fundo */}
      <Loader2 className="w-12 h-12 text-purple-500 animate-spin" />
      <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Carregando Sistema...</p>
    </div>
  );

  // Removido: onConfigClick prop from Login
  if (!session) return <Login />;
  // Fix: Garante que userEmail é string, mesmo se for undefined
