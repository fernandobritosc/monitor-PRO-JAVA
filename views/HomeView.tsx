...
const HomeView: React.FC<HomeViewProps> = ({ records, missaoAtiva, editais, setActiveView }) => {
  ...
  const daysUntilExam = useMemo(() => {
    ...
    // Use .getTime() directly for arithmetic operations
  ...
  const errorData = useMemo(() => {
    const errorsBySubject = activeRecords.reduce((acc: Record<string, number>, r) => {
      ...
      acc[r.materia] = current + (Number(r.total) - Number(r.acertos));
      return acc;
    }, {});
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass rounded-2xl p-6 flex flex-col h-[450px]">
          <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 shrink-0">
            <h3 className="text-lg font-bold flex items-center gap-2"><TrendingUp className="text-purple-400" /> Evolução de Precisão</h3>
            <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-wider text-slate-500 mt-2 md:mt-0">
        ...
        <div className="lg:col-span-1 glass rounded-2xl flex flex-col overflow-hidden border border-white/5 relative group hover:border-cyan-500/30 transition-all h-[450px]">
          {/* Header */}
