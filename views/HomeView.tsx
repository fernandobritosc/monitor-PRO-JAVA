  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      <div className="glass rounded-2xl p-6 border-l-2 border-cyan-500 relative overflow-hidden group">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 relative z-10 w-full">
            ...
            <div className="flex gap-6 text-right shrink-0">
                ...
                <div>
                    <div className="text-2xl font-bold text-white">{todayQuestions}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button onClick={() => setActiveView('REGISTRAR')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-purple-500/50 flex flex-col items-center gap-2">
            <div className="p-3 bg-white/5 rounded-xl group-hover:bg-purple-500 text-purple-400 group-hover:text-white transition-colors"><PlusCircle size={20} /></div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300">Registrar</span>
        </button>
        <button onClick={() => setActiveView('REVISOES')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-orange-500/50 flex flex-col items-center gap-2">
            <div className="p-3 bg-white/5 rounded-xl group-hover:bg-orange-500 text-orange-400 group-hover:text-white transition-colors"><RotateCcw size={20} /></div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300">Revisar</span>
        </button>
        <button onClick={() => setActiveView('DASHBOARD')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-cyan-500/50 flex flex-col items-center gap-2">
            <div className="p-3 bg-white/5 rounded-xl group-hover:bg-cyan-500 text-cyan-400 group-hover:text-white transition-colors"><BarChart3 size={20} /></div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300">Analisar</span>
        </button>
        <button onClick={() => setActiveView('GUIA_SEMANAL')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-green-500/50 flex flex-col items-center gap-2">
            <div className="p-3 bg-white/5 rounded-xl group-hover:bg-green-500 text-green-400 group-hover:text-white transition-colors"><CalendarCheck size={20} /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass rounded-2xl p-6 flex flex-col justify-between">
             <h3 className="text-lg font-bold flex items-center gap-2 mb-4"><Target className="text-cyan-400" size={20} /> Metas da Semana</h3>
             <div className="space-y-4">
                <div>
                    ...
                    <div className="h-2 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-blue-500" style={{ width: `${Math.min((totalHours/22)*100, 100)}%` }} /></div>
                </div>
             </div>
          </div>
          <div className="glass rounded-2xl p-6">
             <h3 className="text-lg font-bold flex items-center gap-2 mb-4"><TrendingUp className="text-green-400" size={20} /> Constância</h3>
