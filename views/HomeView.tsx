...
const HomeView: React.FC<HomeViewProps> = ({ records, missaoAtiva, editais, setActiveView }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      <div className="glass rounded-2xl p-6 border-l-2 border-cyan-500 relative overflow-hidden group"> {/* Reduzido de 3xl p-8 border-l-4 */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button onClick={() => setActiveView('REGISTRAR')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-purple-500/50 flex flex-col items-center gap-3"> {/* Ajustado de /40 p-4 rounded-2xl */}
        ...
        <button onClick={() => setActiveView('REVISOES')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-orange-500/50 flex flex-col items-center gap-3"> {/* Ajustado de /40 p-4 rounded-2xl */}
        ...
        <button onClick={() => setActiveView('DASHBOARD')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-cyan-500/50 flex flex-col items-center gap-3"> {/* Ajustado de /40 p-4 rounded-2xl */}
        ...
        <button onClick={() => setActiveView('GUIA_SEMANAL')} className="group bg-slate-800/30 border border-white/5 p-3 rounded-xl transition-all hover:border-green-500/50 flex flex-col items-center gap-3"> {/* Ajustado de /40 p-4 rounded-2xl */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="glass rounded-2xl p-6 flex flex-col justify-between"> {/* Reduzido de 3xl p-8 */}
          ...
          <div className="glass rounded-2xl p-6"> {/* Reduzido de 3xl p-8 */}
             ...
             <div className="grid grid-cols-2 gap-4 text-center">
                <div className="bg-white/5 rounded-xl p-3 border border-white/5"><div className="text-2xl font-bold text-white">{streak}</div><div className="text-[10px] text-slate-500 uppercase font-bold">Dias Seguidos</div></div> {/* Reduzido de 2xl */}
                <div className="bg-white/5 rounded-xl p-3 border border-white/5"><div className="text-2xl font-bold text-green-400">ATIVO</div><div className="text-[10px] text-slate-500 uppercase font-bold">Status</div></div> {/* Reduzido de 2xl */}
             </div>
          </div>
      </div>
    </div>
