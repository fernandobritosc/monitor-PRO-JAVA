...
const renderAnalysisText = (text: string) => {
    ...
    let processedText = text.replace(tableRegex, (table) => {
        ...
        return `
            <div class="not-prose overflow-x-auto my-4 bg-slate-950/30 rounded-lg border border-slate-700">
...
const extractFinalScore = (text: string | null): string => {
  if (!text) return 'N/A';
  // Regex mais flexível para capturar a nota, ignorando variações de espaços e quebras de linha
  const match = text.match(/##\s*Nota Final\s+\*\*Nota Total:\*\*\s*([^\r\n]+)/);
