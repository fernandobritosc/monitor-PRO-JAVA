...
const extractFinalScore = (text: string | null): string => {
  if (!text) return 'N/A';
  // Regex robusta: Procura por "**Nota Total:**" opcionalmente precedido por "## Nota Final"
  // e captura a nota que pode conter números, pontos, barras e espaços.
  const match = text.match(/(?:##\s*Nota Final\s*)?\*\*Nota Total:\*\*\s*([\d.\s\/]+)/);
  if (match && match[1]) {
