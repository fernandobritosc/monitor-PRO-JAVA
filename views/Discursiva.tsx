...
import { 
  FileEdit, UploadCloud, Loader2, Sparkles, Download, Database, Copy, X, Trash2, Image as ImageIcon, MessageSquare, List, AlertTriangle, ChevronDown, ChevronUp
} from 'lucide-react';
...
const renderAnalysisText = (text: string) => {
    ...
    let processedText = text.replace(tableRegex, (table) => {
        ...
        if (!hasContent) {
            return '<div class="my-4 p-4 bg-slate-950/30 rounded-lg border border-slate-700 text-slate-500 italic text-sm">Nenhum desvio gramatical significativo foi encontrado pela IA.</div>';
        }
                    ...
                    <tbody>
                        ${body.map(row => `
                            <tr class="border-b border-slate-700 last:border-0 hover:bg-white/5">
                                ${row.map(cell => `<td class="p-3 text-sm text-slate-300 font-mono">${cell.replace(/"/g, '')}</td>`).join('')}
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
...
const extractFinalScore = (text: string | null): string => {
  if (!text) return 'N/A';
  const match = text.match(/## Nota Final\n\*\*Nota Total:\*\*\s*(.*?)\n/);
                    ...
                    {history.map(item => (
                        <div key={item.id} className="glass rounded-xl p-4 flex justify-between items-center group hover:bg-white/5 cursor-pointer transition-colors" onClick={() => setSelectedHistory(item)}>
                            ...
                            <div className="flex items-center gap-4 ml-4">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} className="p-2 bg-slate-800 rounded-full text-slate-500 opacity-0 group-hover:opacity-100 hover:text-red-400 hover:bg-red-500/10 transition-all"><Trash2 size={16}/></button>
                            </div>
                        </div>
                    ))}
                </div>
