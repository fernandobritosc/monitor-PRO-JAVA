...
const History: React.FC<HistoryProps> = ({ records, missaoAtiva, editais, onUpdated }) => {
  // Filtros
  ...
  const handleSaveEdit = async () => {
    ...
    try {
        ...
        // --- SALVAR NO BANCO DE QUESTÕES (Se selecionado) ---
        if (saveToBank && editingRecord.dificuldade !== 'Simulado') {
           ...
           if (qError) {
             console.error("Erro ao salvar no banco:", qError);
           }
        }
        ...
        setMsg({ type: 'error', text: "Falha: " + err.message });
    } finally {
        setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      {/* Busca e Filtros */}
      <div className="glass p-4 rounded-xl border border-white/10 flex flex-col md:flex-row gap-4 items-center"> {/* Reduzido de p-5 2xl */}
         <div className="relative flex-1 w-full">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
            <input type="text" placeholder="Filtrar histórico por assunto ou nota..." className="w-full bg-slate-900/30 border border-white/5 rounded-xl pl-12 pr-4 py-3 text-sm focus:ring-2 focus:ring-purple-500/50" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /> {/* Ajustado de /50 e /10 */}
         </div>
      ...
      {showFilters && (
        <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
           <input type="date" className="bg-slate-900/30 border border-white/5 rounded-xl p-3 text-sm" value={dateStart} onChange={e => setDateStart(e.target.value)} /> {/* Ajustado de /900 e /5 */}
           <input type="date" className="bg-slate-900/30 border border-white/5 rounded-xl p-3 text-sm" value={dateEnd} onChange={e => setDateEnd(e.target.value)} /> {/* Ajustado de /900 e /5 */}
        </div>
      )}
             ...
             return (
               <div key={materia} className="glass border border-white/5 rounded-2xl overflow-hidden transition-all duration-300">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-5 bg-slate-900/30 hover:bg-slate-900/50 transition-colors" {/* Ajustado de /40 e /60 */}
                  >
                  ...
                  {isOpen && (
                     <div className="border-t border-white/5 bg-black/20 animate-in slide-in-from-top-2">
                        {recordsInGroup.map(r => (
                           <div key={r.id} className={`p-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors flex flex-col md:flex-row gap-4 justify-between items-start md:items-center group ${deletingId === r.id ? 'opacity-30' : ''}`}> {/* Reduzido de p-5 */}
      ...
      {editingRecord && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="glass w-full max-w-xl rounded-2xl p-6 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar border border-white/10 relative"> {/* Reduzido de 2xl p-8 */}
              ...
              <div className="space-y-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                       ...
                       <div className="flex gap-2">
                          <input type="date" className="flex-1 bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium" value={editForm.data_estudo} onChange={e => setEditForm({...editForm, data_estudo: e.target.value})} /> {/* Ajustado de /900 e /10 */}
                          <input type="text" placeholder="HH:MM" maxLength={5} className="w-24 bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium text-center" value={editForm.tempoHHMM} onChange={handleTimeChange} /> {/* Ajustado de /900 e /10 */}
                       </div>
                    </div>
                    ...
                    <div className="space-y-2">
                       ...
                       <select
                         className="w-full bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium appearance-none" {/* Ajustado de /900 e /10 */}
                 ...
                 <div className="space-y-2">
                    ...
                    <input 
                      type="text" 
                      list="history-topicos-options"
                      className="w-full bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium" {/* Ajustado de /900 e /10 */}
                    ...
                    />
                    {!isSimuladoEdit && (
                      <datalist id="history-topicos-options">
                 ...
                 {/* GRUPO 3: PERFORMANCE */}
                 <div className="bg-slate-900/30 p-6 rounded-2xl border border-white/5"> {/* Ajustado de /30 */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                       <div className="space-y-1">
                          ...
                          <input type="number" className="w-full bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500/50 transition-all text-white font-bold text-lg text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={editForm.acertos} onChange={e => setEditForm({...editForm, acertos: Number(e.target.value)})} /> {/* Ajustado de /900 e /10 */}
                       </div>
                       
                       <div className="space-y-1">
                          <span className="text-[10px] text-slate-500 font-bold uppercase">Total</span>
                          <input type="number" className="w-full bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-bold text-lg text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={editForm.total} onChange={e => setEditForm({...editForm, total: Number(e.target.value)})} /> {/* Ajustado de /900 e /10 */}
                       </div>
                 ...
                 {!isSimuladoEdit && (
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2 bg-slate-900/30 p-1 rounded-xl border border-white/5"> {/* Ajustado de /50 e /10 */}
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                         ...
                         min="1" 
                         max="10" 
                         className="w-full accent-cyan-500 h-2 bg-slate-900/30 rounded-lg appearance-none cursor-pointer" {/* Ajustado de /900 */}
                 ...
                 <div className="space-y-2">
                   ...
                   <textarea
                     className="w-full bg-slate-900/30 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all h-24 text-sm text-slate-300 placeholder-slate-600" {/* Ajustado de /900 e /10 */}
                 ...
                 {!isSimuladoEdit && (
                    <div className="pt-4 border-t border-white/5">
                       <label className="flex items-center gap-4 cursor-pointer group p-4 rounded-xl border border-transparent hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all">
                          <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all shrink-0 ${saveToBank ? 'bg-cyan-500 border-cyan-500' : 'border-slate-600 bg-slate-900/30'}`}> {/* Ajustado de /900 */}
                 ...
                 <button onClick={handleSaveEdit} disabled={loading} className="w-full bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 text-white font-extrabold py-4 rounded-2xl shadow-lg shadow-purple-500/20 transition-all flex items-center justify-center gap-3 disabled:opacity-50"> {/* Reduzido de py-5 shadow-xl */}
