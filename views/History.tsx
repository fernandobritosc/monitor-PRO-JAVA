...
const History: React.FC<HistoryProps> = ({ records, missaoAtiva, editais, onUpdated }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      {/* Busca e Filtros */}
      <div className="glass p-5 rounded-2xl border border-white/10 flex flex-col md:flex-row gap-4 items-center">
         <div className="relative flex-1 w-full">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
            <input type="text" placeholder="Filtrar histórico por assunto ou nota..." className="w-full bg-slate-900/50 border border-white/5 rounded-xl pl-12 pr-4 py-3 text-sm focus:ring-2 focus:ring-purple-500/50" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
         </div>
      ...
      {showFilters && (
        <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
           <input type="date" className="bg-slate-900 border border-white/5 rounded-xl p-3 text-sm" value={dateStart} onChange={e => setDateStart(e.target.value)} />
           <input type="date" className="bg-slate-900 border border-white/5 rounded-xl p-3 text-sm" value={dateEnd} onChange={e => setDateEnd(e.target.value)} />
        </div>
      )}
             ...
             return (
               <div key={materia} className="glass border border-white/5 rounded-2xl overflow-hidden transition-all duration-300">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-5 bg-slate-900/40 hover:bg-slate-900/60 transition-colors"
                  >
                  ...
                  {isOpen && (
                     <div className="border-t border-white/5 bg-black/20 animate-in slide-in-from-top-2">
                        {recordsInGroup.map(r => (
                           <div key={r.id} className={`p-5 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors flex flex-col md:flex-row gap-4 justify-between items-start md:items-center group ${deletingId === r.id ? 'opacity-30' : ''}`}>
      ...
      {editingRecord && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="glass w-full max-w-2xl rounded-3xl p-8 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar border border-white/10 relative">
              ...
              <div className="space-y-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                       ...
                       <div className="flex gap-2">
                          <input type="date" className="flex-1 bg-slate-900 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium" value={editForm.data_estudo} onChange={e => setEditForm({...editForm, data_estudo: e.target.value})} />
                          <input type="text" placeholder="HH:MM" maxLength={5} className="w-24 bg-slate-900 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium text-center" value={editForm.tempoHHMM} onChange={handleTimeChange} />
                       </div>
                    </div>
                 ...
                 <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-medium" 
                 ...
                 <div className="bg-slate-900/30 p-6 rounded-2xl border border-white/5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                       <div className="space-y-1">
                          ...
                          <input type="number" className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500/50 transition-all text-white font-bold text-lg text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={editForm.acertos} onChange={e => setEditForm({...editForm, acertos: Number(e.target.value)})} />
                       </div>
                       
                       <div className="space-y-1">
                          <span className="text-[10px] text-slate-500 font-bold uppercase">Total</span>
                          <input type="number" className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all text-white font-bold text-lg text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={editForm.total} onChange={e => setEditForm({...editForm, total: Number(e.target.value)})} />
                       </div>
                 ...
                 <button onClick={handleSaveEdit} disabled={loading} className="w-full bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 text-white font-extrabold py-5 rounded-2xl shadow-xl shadow-purple-500/20 transition-all flex items-center justify-center gap-3 disabled:opacity-50">
