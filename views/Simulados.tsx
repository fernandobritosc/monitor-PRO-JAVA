...
// Fix: Use React.FC to properly handle "key" prop and adjust onUpdate return type
const SimuladoSubjectRow: React.FC<{ 
  record: StudyRecord, 
  onUpdate: (id: string, updates: Partial<StudyRecord>) => void | Promise<void> 
}> = ({ 
  record, 
  onUpdate 
}) => {
  ...
  return (
    <div className="bg-slate-900/30 rounded-lg p-2 border border-white/5 hover:bg-white/5 transition-colors"> {/* Ajustado de /40 xl p-3 */}
        {isEditing ? (
             <div className="space-y-3">
                 ...
                 <div className="flex items-center gap-2">
                     <div className="flex-1">
                         ...
                         <input 
                           type="number" 
                           className={`w-full bg-slate-950/30 border ${isInvalid ? 'border-red-500 text-red-400' : 'border-white/10 text-green-400'} rounded px-2 py-1.5 text-center font-bold focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`} {/* Ajustado de /950 */}
                           value={localAcertos} 
                     ...
                     <div className="flex-1">
                         ...
                         <input 
                           type="number" 
                           className="w-full bg-slate-950/30 border border-white/10 rounded px-2 py-1.5 text-center font-bold text-white focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" {/* Ajustado de /950 */}
                           value={localTotal} 
...
const Simulados: React.FC<SimuladosProps> = ({ records, missaoAtiva, onSaved, setActiveView }) => {
  ...
  const deleteGroup = async (group: typeof groupedSimulados[0]) => {
     ...
     const { error } = await supabase.from('registros_estudos').delete().in('id', idsToDelete);
     
     if (error) alert('Erro ao excluir: ' + error.message);
     else onSaved();
  };

  const handleUpdateRecord = async (id: string, updates: Partial<StudyRecord>) => {
      const { error } = await supabase.from('registros_estudos').update(updates).eq('id', id);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500 pb-20">
       ...
       {groupedSimulados.length === 0 ? (
         <div className="glass rounded-2xl p-12 md:p-20 text-center space-y-6 border border-white/5"> {/* Reduzido de 3xl */}
         ...
         <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               <div className="glass p-4 rounded-xl border-b-4 border-yellow-500"> {/* Reduzido de p-5 2xl */}
               ...
               <div className="glass p-4 rounded-xl border-b-4 border-cyan-500"> {/* Reduzido de p-5 2xl */}
               ...
               <div className="glass p-4 rounded-xl border-b-4 border-purple-500"> {/* Reduzido de p-5 2xl */}
               ...
               <div className="glass p-4 rounded-xl border-b-4 border-green-500"> {/* Reduzido de p-5 2xl */}
            ...
            {/* Chart */}
            <div className="glass rounded-2xl p-6"> {/* Reduzido de 3xl p-6 md:p-8 */}
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 gap-4">
                  {groupedSimulados.map(group => {
                    ...
                    return (
                        <div key={group.id} className={`glass rounded-xl overflow-hidden border-l-2 ${colorClass}`}> {/* Reduzido de 2xl border-l-4 */}
                            {/* Card Header (Click to Expand) */}
                            <div className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-white/[0.02] transition-all cursor-pointer" onClick={() => toggleExpand(group.id)}> {/* Reduzido de p-6 */}
                            ...
                            {/* Expanded Details */}
                            {isExpanded && (
                                <div className="bg-slate-900/30 border-t border-white/5 p-4 animate-in slide-in-from-top-2"> {/* Ajustado de /50 */}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                        {group.records.map(record => (
                                            <SimuladoSubjectRow key={record.id} record={record} onUpdate={handleUpdateRecord} />
                                        ))}
                                    </div>
                                </div>
                            )}
