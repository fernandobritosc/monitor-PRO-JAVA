...
Você é um professor especialista em concursos, membro de uma banca examinadora rigorosa como a FGV/Cebraspe. Sua tarefa é analisar a IMAGEM de uma página de prova com MÁXIMA ATENÇÃO.

1.  **IDENTIFICAÇÃO EXAUSTIVA:** Examine a imagem CUIDADOSAMENTE, de cima a baixo, e identifique **ABSOLUTAMENTE TODAS** as questões numeradas presentes. Não pule nenhuma, mesmo que a formatação seja estranha ou a imagem tenha marcações.
2.  **ANÁLISE INDIVIDUAL:** Para CADA questão identificada, determine a alternativa correta (A, B, C, D ou E).
3.  **JUSTIFICATIVA DETALHADA:** Forneça uma justificativa técnica/jurídica detalhada para a resposta correta, explicando o porquê dela estar certa.
4.  **SAÍDA ESTRITA:** Sua resposta DEVE ser um JSON válido contendo uma lista de objetos, seguindo a estrutura definida. Se não encontrar NENHUMA questão, retorne um array JSON vazio \`[]\`.
`;
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // **LÓGICA DE ANÁLISE DE IMAGEM (MAIS ROBUSTA)**
        // Converte a página do PDF em uma imagem para análise, funcionando com qualquer tipo de PDF.
        const viewport = page.getViewport({ scale: 2 });
        const canvas = document.createElement('canvas');
        const context = canvas.getContext('2d');
        if (!context) {
            console.warn(`Página ${i} não pôde obter contexto 2D, pulando.`);
            continue;
        }
    ...
    <div className="space-y-6">
        ...
        {!results && !processing && (
            <div className="glass p-8 rounded-2xl text-center border-2 border-dashed border-slate-700 hover:border-cyan-500 transition-all">
                ...
                <p className="text-slate-400 mb-6">A IA irá analisar a imagem de cada página para gerar um gabarito comentado.</p>
