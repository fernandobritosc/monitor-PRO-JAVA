...
import { UploadCloud, Loader2, Sparkles, Download, FileCheck, Check, X, AlertTriangle, ChevronDown, ChevronUp, FileText, Trash2, Save, ArrowLeft, History, BarChart, Type as TypeIcon, PlusCircle } from 'lucide-react';
  ...
  const handleManualAddAndAnalyze = async () => {
    ...
    try {
        ...
        const response = await ai.models.generateContent({
          model: 'gemini-3-flash-preview',
          contents: prompt,
          config: { responseMimeType: 'application/json', responseSchema: singleResponseSchema }
        });

        const newQuestionPartial = JSON.parse(response.text || '{}');
        ...
        <div className="space-y-6 animate-in fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                ...
                <div className="flex gap-2 self-start md:self-center">
                    <button onClick={() => setShowManualAddModal(true)} className="px-4 py-2 bg-slate-800 hover:bg-cyan-600 text-white text-xs font-bold rounded-xl flex items-center gap-2"><PlusCircle size={14}/> Adicionar Questão</button>
                    <button onClick={generatePDF} className="px-4 py-2 bg-slate-800 hover:bg-blue-600 text-white text-xs font-bold rounded-xl flex items-center gap-2"><Download size={14}/> PDF</button>
                    <button onClick={handleUpdate} className="px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-xs font-bold rounded-xl flex items-center gap-2"><Save size={14}/> Salvar</button>
                </div>
            </div>
