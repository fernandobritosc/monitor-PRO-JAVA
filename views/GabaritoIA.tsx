...
Você é um professor especialista em concursos, membro de uma banca examinadora rigorosa como a FGV/Cebraspe. Analise o TEXTO a seguir, que foi extraído de uma página de prova. Sua tarefa é:
1. Identificar CADA questão numerada presente no texto.
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // **NOVA LÓGICA: Extração de texto puro**
        const textContent = await page.getTextContent();
        const pageText = textContent.items.map((item: any) => item.str).join(' ');

        // Pula páginas sem texto para economizar API
        if (!pageText.trim() || pageText.length < 50) {
            continue;
        }

        const promptForPage = `${AI_PROMPT_GABARITO_TEXT}\n\n--- TEXTO EXTRAÍDO DA PÁGINA PARA ANÁLISE ---\n${pageText}`;
        
        const response = await ai.models.generateContent({
          model: 'gemini-3-flash-preview',
          contents: promptForPage,
          config: { 
    ...
    <div className="space-y-6">
        ...
        {!results && !processing && (
            <div className="glass p-8 rounded-2xl text-center border-2 border-dashed border-slate-700 hover:border-cyan-500 transition-all">
                ...
                <p className="text-slate-400 mb-6">A IA irá ler o texto puro do seu PDF para gerar um gabarito comentado.</p>
        ...
        {processing && (
            <div className="glass p-8 rounded-2xl text-center"><Loader2 size={48} className="mx-auto animate-spin text-cyan-400 mb-6"/>
                ...
                <p className="text-slate-400">Página {progress.current} de {progress.total}. Extraindo texto e gerando gabarito.</p>
