                    ...
                    <div className="space-y-2">
                        {savedGabaritos.map(item => (
                            <div key={item.id} className="glass rounded-xl p-4 flex justify-between items-center group hover:bg-white/5 cursor-pointer" onClick={() => { setSelectedGabarito(item); setUserAnswers(item.user_answers_json); setOfficialAnswers(item.official_answers_json); }}>
                                ...
                                <div className="flex items-center gap-2"><div className="text-lg font-bold text-cyan-400">{Object.keys(item.user_answers_json).filter(k => item.user_answers_json[k] === item.results_json.find(r => r.numero_questao === Number(k))?.alternativa_correta_ia).length}/{Object.keys(item.user_answers_json).length}</div><button onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} className="p-2 bg-slate-800 rounded-full text-slate-500 opacity-20 group-hover:opacity-100 hover:text-red-400"><Trash2 size={16}/></button></div>
                            </div>
                        ))}
                    </div>
                )}
