  ...
  const runDiagnostics = async () => {
    ...
    try {
        ...
        if (session) {
            ...
            if (recError) log(`❌ Erro Contagem Registros: ${recCount.message}`);
...
`;
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      {activeTab === 'mission' && (
        <div className="glass rounded-2xl p-6 shadow-xl animate-in slide-in-from-right-2"> {/* Reduzido de 3xl p-8 */}
            ...
            {refreshing && groupedMissions.length === 0 ? (
                ...
                {groupedMissions.map(m => {
                    ...
                    return (
                    <div key={m.concurso} className={`p-5 rounded-xl border transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 ${isActive ? 'bg-cyan-500/5 border-cyan-500/30' : 'bg-slate-900/40 border-white/5 hover:border-white/10'}`}> {/* Reduzido de p-6 2xl */}
      ...
      {activeTab === 'import' && (
         <div className="glass rounded-2xl p-6 shadow-xl animate-in slide-in-from-right-2 space-y-6"> {/* Reduzido de 3xl p-8 */}
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                 ...
                 <div className="relative w-full md:w-64">
                    ...
                    <input 
                        ...
                        className="w-full bg-slate-900/30 border border-white/5 rounded-xl pl-9 pr-4 py-2 text-xs" {/* Ajustado de /900 e /10 */}
                        value={importSearch} 
             ...
             {loadingTemplates ? (
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTemplates.map(tmpl => (
                        <div key={tmpl.id} className="bg-slate-900/30 border border-white/5 rounded-2xl p-6 hover:border-blue-500/50 transition-all group hover:bg-slate-900/50 relative"> {/* Ajustado de /40 e /60 */}
      ...
      {activeTab === 'admin' && isAdmin && (
        <div className="glass rounded-2xl p-6 border border-purple-500/30 relative overflow-hidden animate-in slide-in-from-right-2"> {/* Reduzido de 3xl p-8 */}
          {approvalMsg && (
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <div className="relative flex-1 md:w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={14} />
                    <input type="text" placeholder="Filtrar usuários..." className="w-full bg-slate-900/30 border border-white/5 rounded-xl pl-9 pr-4 py-2 text-xs" value={userSearch} onChange={e => setUserSearch(e.target.value)} /> {/* Ajustado de /900 e /10 */}
                </div>
            </div>
          </div>
          <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
            {loadingUsers ? <Loader2 className="animate-spin mx-auto text-purple-400" /> : filteredUsers.map(u => (
              <div key={u.id} className="bg-slate-950/30 p-3 rounded-xl border border-white/5 flex items-center justify-between"> {/* Ajustado de /950 */}
