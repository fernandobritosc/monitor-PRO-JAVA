...
const Configurar: React.FC<ConfigurarProps> = ({ editais, missaoAtiva, onUpdated, setMissaoAtiva }) => {
  ...
  const sqlScript = useMemo(() => {
     return `-- 1. TABELA DE PERFIS (OPCIONAL NO MODO LIVRE, MAS RECOMENDADO)
...
-- 3. REGRAS DE ACESSO E COMPARTILHAMENTO (IMPORTANTE!)
...
-- Editais (Permite leitura pública para clonagem, mas edição restrita ao dono)
...
-- Registros de Estudos (Privado)
create policy "Users manage records" on registros_estudos for all using (auth.uid() = user_id);

-- 4. GARANTIR SEU ADMIN (Caso queira voltar ao modo restrito futuramente)
update profiles set is_admin = true, approved = true where email = '${currentUserEmail || 'seu_email@exemplo.com'}';`;
  }, [currentUserEmail]);
