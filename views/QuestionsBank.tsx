...
const QuestionsBank: React.FC<QuestionsBankProps> = ({ missaoAtiva, editais }) => {
  ...
  const reviewQueue = useMemo(() => {
    ...
    return [...filtered].sort((a: Question, b: Question) => {
      const relA = Number(a.relevancia) || 0;
      const relB = Number(b.relevancia) || 0;
      if (relB !== relA) {
        return relB - relA;
      }
      // FIX: The error is caused by attempting to subtract Date objects directly.
      // In TypeScript, this is not allowed. We must convert them to numbers first using .getTime().
      // FIX: Changed to use .getTime() to get numeric timestamp for subtraction.
