...
export const QuestionsBank: React.FC<QuestionsBankProps> = ({ missaoAtiva, editais }) => {
  ...
  const fetchQuestions = async () => {
    ...
    const { data, error } = await supabase.from('questoes_revisao').select('*').eq('user_id', user.id).eq('concurso', missaoAtiva);
  ...
  const weakPoints = useMemo(() => {
     const pending = questions.filter(q => q.status !== 'Concluída');
     const stats: Record<string, { count: number, totalRelevance: number }> = {};
     pending.forEach(q => {
        if (!stats[q.materia]) stats[q.materia] = { count: 0, totalRelevance: 0 };
        stats[q.materia].count++;
        stats[q.materia].totalRelevance += q.relevancia;
     });
     return Object.entries(stats)
        .map(([materia, data]) => ({ materia, ...data, score: data.count * data.totalRelevance }))
        .sort((a, b) => b.score - a.score)
        .slice(0, 3);
  }, [questions]);

  // FILA INTELIGENTE: Ordenação por Relevância > Status > Data
  const reviewQueue = useMemo(() => {
      ...
      const filtered = questions.filter(q => {
        ...
        const matchesMateria = filterMateria === 'Todas' || q.materia === filterMateria;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('questoes_revisao').update(questionPayload).eq('id', isEditing);
      ...
      {showForm && (
        <div className="glass p-8 rounded-3xl border border-white/10 animate-in slide-in-from-top-4 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
             {/* ... conteúdo do form (simplificado para brevidade) ... */}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="text-xs text-slate-400">Matéria</label><select className="w-full bg-slate-900 p-2 rounded-lg border border-white/5 mt-1" value={formData.materia} onChange={e => setFormData({...formData, materia: e.target.value})}><option value="">Selecione</option>{editais.filter(e => e.concurso === missaoAtiva).map(e => e.materia).sort().map(m => <option key={m}>{m}</option>)}</select></div>
                <div><label className="text-xs text-slate-400">Assunto</label><input type="text" list="topicos-list" className="w-full bg-slate-900 p-2 rounded-lg border border-white/5 mt-1" value={formData.assunto} onChange={e => setFormData({...formData, assunto: e.target.value})} /><datalist id="topicos-list">{topicosDisponiveis.map(t=><option key={t} value={t}/>)}</datalist></div>
             </div>
             <div><label className="text-xs text-slate-400">Anotações / Link</label><textarea className="w-full bg-slate-900 p-2 rounded-lg border border-white/5 mt-1 h-24" value={formData.anotacoes} onChange={e => setFormData({...formData, anotacoes: e.target.value})} /></div>
             <div className="grid grid-cols-3 gap-4">
                 <div><label className="text-xs text-slate-400">Relevância</label><input type="range" min="1" max="10" className="w-full mt-1 accent-cyan-500" value={formData.relevancia} onChange={e => setFormData({...formData, relevancia: Number(e.target.value)})} /></div>
                 <div><label className="text-xs text-slate-400">Status</label><select className="w-full bg-slate-900 p-2 rounded-lg border border-white/5 mt-1" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as any})}><option>Pendente</option><option>Em andamento</option><option>Concluída</option></select></div>
                 <div><label className="text-xs text-slate-400">Tags</label><input type="text" placeholder="erro, lei, jurisprudência" className="w-full bg-slate-900 p-2 rounded-lg border border-white/5 mt-1" value={formData.tags} onChange={e => setFormData({...formData, tags: e.target.value})} /></div>
             </div>
             <div className="flex justify-end gap-3 pt-3 border-t border-white/10"><button type="button" onClick={handleCancel} className="px-4 py-2 rounded-lg border border-white/10 text-sm font-bold">Cancelar</button><button type="submit" className="px-4 py-2 rounded-lg bg-cyan-600 text-sm font-bold">Salvar</button></div>
          </form>
        </div>
      )}
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <div className="flex gap-2 items-center w-full md:w-auto">
               <div className="relative flex-1">
                   ...
                   <input type="text" placeholder="Filtrar na fila..." className="w-full bg-slate-900/50 border border-white/10 rounded-xl pl-9 pr-3 py-2 text-sm" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
               </div>
               <select className="bg-slate-900/50 border border-white/10 rounded-xl px-3 py-2 text-sm" value={filterMateria} onChange={e => setFilterMateria(e.target.value)}>
