...
import { streamAIContent, AIProvider, speakTextWithGemini } from '../services/aiService';
...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Eye, Sparkles, AlertTriangle, Volume2, Info, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus, ChevronUp, Headphones, Square
} from 'lucide-react';
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const generateAIExplanation = async (prompt: string) => {
    ...
    // Para áudio neural funcionar, a chave Gemini deve estar disponível
    // Se a explicação for gerada pelo Groq, ainda usamos Gemini para TTS
  ...
  const handlePlayNeural = async () => {
      ...
      const cancel = await speakTextWithGemini(
          aiStreamText,
          key,
          () => setIsPlayingNeural(true),
          () => setIsPlayingNeural(false),
  ...
  useEffect(() => {
      return () => { if (stopNeural) stopNeural(); setIsPlayingNeural(false); };
                  ...
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 bg-slate-900/80 border border-white/10 rounded-2xl p-6 animate-in slide-in-from-bottom-4">
                          <div className="flex justify-between items-center mb-4">
                              ...
                              {/* NOVO: Botão de Podcast Neural */}
                              {aiStreamText && !aiLoading && (
                                <button 
                                    onClick={handlePlayNeural}
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${isPlayingNeural ? 'bg-red-500/20 text-red-400 border-red-500/50' : 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 hover:bg-cyan-500/30'}`}
                                >
                                    {isPlayingNeural ? <><Square size={10} className="fill-current" /> Parar Áudio</> : <><Headphones size={12} /> Ouvir (Voz Neural)</>}
                                </button>
                              )}
                          </div>
