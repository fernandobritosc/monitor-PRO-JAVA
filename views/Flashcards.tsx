...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Shuffle, Eye, Sparkles, AlertTriangle, Settings, Volume2
} from 'lucide-react';
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const [editingCard, setEditingCard] = useState<LocalFlashcard | null>(null);
  const [newCard, setNewCard] = useState({ front: '', back: '', materia: '' });
  const [similarityThreshold] = useState(0.8);
  const [isSpeaking, setIsSpeaking] = useState(false);

  const materias = useMemo(() => { const unique = Array.from(new Set(editais.map(e => e.materia))); return ['Todas', ...unique]; }, [editais]);
  ...
  const saveCard = async () => {
    if (!newCard.front.trim() || !newCard.back.trim() || !newCard.materia) { alert('Preencha todos os campos'); return; }
    if (checkForDuplicates(newCard.front, newCard.materia)) { alert('Flashcard duplicado detectado!'); return; }
    try {
      const userId = (await supabase.auth.getUser()).data.user?.id;
      if (!userId) throw new Error('Usuário não autenticado');
      const { error } = await supabase.from('flashcards').insert([{ user_id: userId, materia: newCard.materia, front: newCard.front, back: newCard.back, status: 'novo' as LocalFlashcard['status'] }]);
      if (error) throw error;
      setNewCard({ front: '', back: '', materia: '' }); loadFlashcards();
    } catch (error) { alert('Erro ao salvar flashcard'); }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'study' && (
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            <div className="flex flex-col gap-6">
              ...
              {/* BARRA DE FERRAMENTAS DE IA (DEDICADA) */}
              <div className="bg-slate-900/40 border border-purple-500/20 rounded-xl p-4 flex flex-col md:flex-row justify-between items-center gap-4">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-[10px] text-slate-400 flex gap-2 mt-0.5">
                            <span className={getGeminiKey() ? 'text-green-400' : 'text-slate-600'}>Gemini: {getGeminiKey() ? 'ON' : 'OFF'}</span>
                            <span>•</span>
                            <span className={getGroqKey() ? 'text-green-400' : 'text-slate-600'}>Groq: {getGroqKey() ? 'ON' : 'OFF'}</span>
                        </div>
                    </div>
                 </div>
                 <div className="flex items-center gap-2 bg-slate-950/50 p-1 rounded-lg border border-white/5">
                    ...
                    <button onClick={() => setSelectedAI('gemini')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${selectedAI === 'gemini' ? 'bg-purple-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}>Gemini</button>
                    <button onClick={() => setSelectedAI('groq')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${selectedAI === 'groq' ? 'bg-purple-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}>Groq</button>
                 </div>
              </div>
            </div>
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                <div className={`relative w-full h-96 cursor-pointer transform-style-3d transition-all duration-500 ${isFlipped ? 'rotate-y-180' : ''}`} onClick={() => !isSpeaking && setIsFlipped(!isFlipped)}>
                  ...
                  <div className={`absolute inset-0 backface-hidden bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-cyan-500/30 rounded-2xl p-8 flex flex-col ${isFlipped ? 'opacity-0' : 'opacity-100'}`}>
                    <div className="flex justify-between items-start mb-6">
                        <span className="px-4 py-1 bg-cyan-900/50 text-cyan-400 rounded-full text-xs font-bold">{currentCard.materia}</span>
                ...
                <div className="flex justify-center gap-4 mt-8">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => updateCardStatus(currentCard.id, 'revisando')} className="px-4 py-3 bg-yellow-600/20 hover:bg-yellow-600/30 text-yellow-400 rounded-xl font-bold flex items-center gap-2"><RotateCcw size={14} /> Revisar</button>
                    <button onClick={() => updateCardStatus(currentCard.id, 'aprendendo')} className="px-4 py-3 bg-green-600/20 hover:bg-green-600/30 text-green-400 rounded-xl font-bold flex items-center gap-2"><CheckCircle2 size={14} /> Aprendendo</button>
                  </div>
        ...
        {/* MANAGE & COMMUNITY TABS (MANTIDAS IGUAIS, APENAS RENDERIZADAS) */}
        {activeTab === 'manage' && (
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            <div className="bg-slate-900/30 border border-white/5 rounded-2xl p-6"><h4 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Plus size={18} /> Criar Novo Flashcard</h4><div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4"><div><label className="block text-sm font-bold text-slate-400 mb-2">Matéria</label><select value={newCard.materia} onChange={(e) => setNewCard({...newCard, materia: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 outline-none"><option value="">Selecione...</option>{materias.filter(m => m !== 'Todas').map(m => <option key={m} value={m}>{m}</option>)}</select></div><div className="md:col-span-2"><label className="block text-sm font-bold text-slate-400 mb-2">Pergunta</label><input type="text" value={newCard.front} onChange={(e) => setNewCard({...newCard, front: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 outline-none" placeholder="Pergunta..." /></div></div><div className="mb-4"><label className="block text-sm font-bold text-slate-400 mb-2">Resposta</label><textarea value={newCard.back} onChange={(e) => setNewCard({...newCard, back: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 outline-none min-h-[100px]" placeholder="Resposta..." /></div><button onClick={saveCard} className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-bold flex items-center gap-2"><Save size={16} /> Salvar</button></div>
            {filteredCards.length === 0 ? <div className="text-center py-16 border-2 border-dashed border-slate-800 rounded-2xl"><p className="text-slate-500">Nada aqui.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{filteredCards.map(card => (<div key={card.id} className="bg-slate-900/30 border border-white/5 rounded-2xl p-6 hover:border-cyan-500/30 transition-all group relative"><div className="flex justify-between items-start mb-4"><span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-900/30 text-blue-400">{card.status}</span><div className="flex gap-2"><button onClick={() => deleteCard(card.id)} className="p-2 bg-red-900/20 hover:bg-red-900/30 rounded-lg text-red-400"><Trash2 size={14} /></button></div></div><h4 className="text-lg font-bold text-white mb-2">{card.front}</h4><p className="text-sm text-slate-400 mb-4 line-clamp-3">{card.back}</p><div className="flex justify-between items-center text-xs text-slate-600"><span>{card.materia}</span></div></div>))}</div>}
          </div>
        )}
