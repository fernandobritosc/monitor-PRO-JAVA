          ...
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                <div className={`relative w-full h-80 md:h-96 cursor-pointer transform-style-3d transition-all duration-500 ${isFlipped ? 'rotate-y-180' : ''}`} onClick={() => !isSpeaking && setIsFlipped(!isFlipped)}>
                  {/* Frente */}
                  <div className={`absolute inset-0 backface-hidden bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-cyan-500/30 rounded-2xl p-6 md:p-8 flex flex-col ${isFlipped ? 'opacity-0' : 'opacity-100'}`}>
                    ...
                    <div className="flex-1 flex items-center justify-center"><p className="text-xl md:text-3xl text-center text-white leading-relaxed">{currentCard.front}</p></div>
                    <div className="mt-6 text-center text-slate-500 text-sm">Clique para virar</div>
                  </div>
                  {/* Verso */}
                  <div className={`absolute inset-0 backface-hidden rotate-y-180 bg-gradient-to-br from-purple-900/80 to-purple-800/80 border-2 border-purple-500/30 rounded-2xl p-6 md:p-8 flex flex-col ${isFlipped ? 'opacity-100' : 'opacity-0'}`}>
                    <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex-1 flex items-center justify-center overflow-y-auto custom-scrollbar"><p className="text-lg md:text-2xl text-center text-white leading-relaxed">{currentCard.back}</p></div>
                  </div>
                </div>
