...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Shuffle, Eye, Sparkles, AlertTriangle, Settings, Volume2, Info, List, RefreshCw, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus
} from 'lucide-react';
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const [followUpQuery, setFollowUpQuery] = useState(""); // NOVO: Estado para pergunta de aprofundamento
  ...
  const handleCardResult = async (status: LocalFlashcard['status']) => {
    ...
    if (nextIndex < studyQueue.length) {
        setIsFlipped(false); setAiStreamText(""); setFollowUpQuery(""); setCurrentCardIndex(nextIndex);
    } else {
...
Resposta esperada: "${currentCard.back}"`;
  ...
  // --- NOVA FUNÇÃO: CHAT DE APROFUNDAMENTO (FOLLOW-UP) ---
  const handleSendFollowUp = async () => {
      ...
      // Adiciona a pergunta do usuário ao histórico visual antes da resposta
      const questionText = `\n\n🤔 **Sua Pergunta:** ${followUpQuery}\n\n🤖 **Tutor:** `;
      setAiStreamText(prev => prev + questionText);
      setFollowUpQuery(""); // Limpa input

      // Prompt com contexto completo e restrição de escopo
      const contextPrompt = `
      ATENÇÃO: Você é um Tutor Especialista em Concursos Públicos.
      
      CONTEXTO DO ESTUDO:
      Matéria: ${currentCard.materia}
      Card Frente: "${currentCard.front}"
      Card Verso: "${currentCard.back}"
      ...
      1. Responda apenas à nova dúvida do aluno.
      2. Mantenha o foco estritamente no tema do card e na matéria.
      3. Se a pergunta fugir do assunto (ex: futebol, receitas, piadas fora de contexto), responda educadamente: "Vamos manter o foco no estudo deste tópico para garantir sua aprovação."
      4. Seja didático, direto e motive o aluno.
      `;
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                ...
                <div className="mt-6 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button 
                            onClick={() => { if (currentCardIndex > 0) { setCurrentCardIndex(currentCardIndex - 1); setIsFlipped(false); setAiStreamText(""); setFollowUpQuery(""); } }} 
                        ...
                        <button 
                            onClick={() => { if (currentCardIndex < studyQueue.length - 1) { setCurrentCardIndex(currentCardIndex + 1); setIsFlipped(false); setAiStreamText(""); setFollowUpQuery(""); } }} 
                ...
                <div className="mt-6">
                  ...
                  {/* Área de Resposta e Chat de Aprofundamento */}
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 bg-slate-900/80 border border-white/10 rounded-2xl p-6 animate-in slide-in-from-bottom-4">
                          <div className="flex items-center gap-2 mb-2 text-purple-400 font-bold text-xs uppercase tracking-widest">
                              <Sparkles size={12} /> Explicação ({getActiveProviderName()})
                          </div>
                          
                          {/* Conteúdo da IA */}
                          <div className="text-slate-300 whitespace-pre-wrap leading-relaxed text-sm mb-6 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                              {aiStreamText}
                          </div>

                          {/* Campo de Dúvida Extra (Aparece apenas se tiver explicação e não estiver carregando) */}
                          {aiStreamText && !aiLoading && (
                              <div className="pt-4 border-t border-white/10 animate-in fade-in slide-in-from-bottom-2">
                                  <label className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2">
                                      <MessageSquarePlus size={12} /> Dúvida Específica sobre a explicação?
                                  </label>
                                  <div className="flex gap-2">
                                      <input 
                                          type="text" 
                                          className="flex-1 bg-slate-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all"
                                          placeholder="Ex: Pode dar mais um exemplo?"
                                          value={followUpQuery}
                                      ...
                                      <button 
                                          ...
                                          className="bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed text-white p-2.5 rounded-xl transition-all shadow-lg shadow-purple-500/10"
                                      >
                                          <Send size={18} />
                                      </button>
                                  </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[120] flex items-center justify-center p-4">
            <div className="bg-slate-950 border border-slate-700 w-full max-w-5xl rounded-2xl p-6 relative shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-white/10">
                    ...
                    <button onClick={() => handleImportDeck(previewDeck)} disabled={importingState.loading} className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-bold shadow-lg shadow-cyan-500/20 flex items-center gap-2">
