...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Eye, Sparkles, AlertTriangle, Volume2, Info, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus, ChevronUp, Headphones, Square, Mic2
} from 'lucide-react';
...
// SCRIPT SQL CORRIGIDO: Adiciona colunas ai_explanation e original_audio_id
...
CREATE TABLE IF NOT EXISTS public.flashcards (
  ...
  back text NOT NULL,
  ai_explanation text,
  original_audio_id text, -- NOVO CAMPO: ID do card original para compartilhar áudio
...
-- ATUALIZAÇÃO PARA TABELAS EXISTENTES
...
WHERE id IN (
  SELECT id FROM (
    SELECT id,
    ROW_NUMBER() OVER (
      PARTITION BY user_id, materia, front
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const handlePodcastDuo = async () => {
      ...
      const cancel = await generatePodcastAudio(
          ...
          () => { setIsPlayingNeural(true); setPodcastStatus("No ar!"); },
