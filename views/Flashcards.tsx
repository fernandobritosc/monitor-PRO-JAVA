...
import { streamAIContent, AIProvider, speakTextWithGemini, generatePodcastAudio, handlePlayRevisionAudio, deleteCachedAudio } from '../services/aiService';
...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Eye, Sparkles, AlertTriangle, Volume2, Info, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus, ChevronUp, Headphones, Square, Mic2, FileAudio, RefreshCw
} from 'lucide-react';
...
// SCRIPT SQL COMPLETO: Tabela + Colunas Novas + Storage + Policies
...
CREATE TABLE IF NOT EXISTS public.flashcards (
  ...
  front text NOT NULL,
  back text NOT NULL,
  ai_explanation text, -- Explicação da IA cacheada
  original_audio_id text, -- ID do áudio original (para clonagem eficiente)
...
-- Policies do Storage (Upload/Delete/Read)
...
const CustomFilterDropdown: React.FC<CustomFilterDropdownProps> = ({ label, value, options, onChange, icon, widthClass = "w-48", colorClass = "focus:ring-cyan-500", align = 'left' }) => {
  ...
  return (
    <div className={`relative ${widthClass}`} ref={dropdownRef}>
      ...
      {isOpen && (<div className={`absolute top-full mt-2 ${align === 'right' ? 'right-0' : 'left-0'} max-h-[300px] overflow-y-auto bg-[#1a1d26] border border-white/10 rounded-xl shadow-2xl z-[9999] custom-scrollbar animate-in fade-in slide-in-from-top-2`} style={{ width: '300px', maxWidth: '90vw' }}><div onClick={() => { onChange(label.includes('Matéria') ? 'Todas' : 'Todos'); setIsOpen(false); }} className="p-3 border-b border-white/5 hover:bg-white/5 cursor-pointer transition-colors"><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Limpar Seleção</span></div>{options.map((opt, idx) => (<div key={idx} onClick={() => { onChange(opt); setIsOpen(false); }} className={`p-3 border-b border-white/5 last:border-0 hover:bg-white/5 cursor-pointer transition-colors flex items-start gap-2 ${value === opt ? 'bg-cyan-500/10' : ''}`}><div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${value === opt ? 'bg-cyan-400' : 'bg-slate-700'}`} /><span className={`text-xs font-medium leading-relaxed flex-1 ${value === opt ? 'text-cyan-100' : 'text-slate-300'}`}>{opt}</span></div>))}</div>)}
    </div>
  );
};
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const [isSyncing, setIsSyncing] = useState(false); // Novo state para loading do sync
  ...
  // Sincroniza cache de podcasts ao carregar
  // AGORA LISTA TUDO E FILTRA NO CLIENTE PARA EVITAR PROBLEMAS DE INDEXAÇÃO
  const syncPodcastCache = async () => {
      ...
      try {
          ...
          console.error("Erro exceção sync podcast:", e);
      } finally {
          setIsSyncing(false);
      }
  };
  ...
  const loadCommunityDecks = async () => {
    ...
    try {
      // IMPORTANTE: Agora selecionamos ai_explanation e original_audio_id para clonagem
      const { data, error } = await supabase.from('flashcards').select('materia, assunto, front, back, id, status, created_at, ai_explanation, original_audio_id').not('user_id', 'eq', '00000000-0000-0000-0000-000000000000');
      if (error) throw error;
      const decksMap = new Map();
      data?.forEach((card: any) => {
        const signature = `${normalizeText(card.materia)}||${normalizeText(card.front)}`;
        if (userCardSignatures.has(signature)) return;
  ...
  const importCards = async (cardsToImport: any[], type: 'deck' | 'topic' | 'single', topicName?: string) => {
      ...
      try {
          ...
          const payload = cardsToImport.map(c => ({ 
              ...
              front: c.front, 
              back: c.back,
              ai_explanation: c.ai_explanation, // Copia a explicação existente
              // LÓGICA DE AUDIO COMPARTILHADO:
              // Se o card original já aponta para outro (é uma cópia), usamos o ponteiro dele.
              // Se não, usamos o ID do card original como ponteiro.
          ...
          if (type !== 'single') alert(`${payload.length} cards importados com sucesso! O áudio original foi vinculado.`);
  ...
  const generateAIExplanation = async (prompt: string) => {
    ...
    await streamAIContent(prompt, { 
      ...
      onComplete: async () => {
        setAiLoading(false);
        // SALVA A EXPLICAÇÃO NO BANCO ASSIM QUE TERMINAR
        if (currentCard && accumulatedText) {
            // Atualiza estado local para persistir se mudar de card e voltar sem reload
  ...
  const saveOrUpdateCard = async () => {
    ...
    try {
      if (editingId) {
        // Se editou, removemos explicação e áudio antigos pois podem não bater mais
        // Também removemos o link do áudio original, pois o conteúdo mudou
        const { error } = await supabase.from('flashcards').update({ 
            materia: newCard.materia, 
            assunto: newCard.assunto, 
            front: newCard.front, 
            back: newCard.back,
            ai_explanation: null, // Reseta a explicação
            original_audio_id: null // Reseta o link de áudio compartilhado
        }).eq('id', editingId);
        
        if (error) throw error; 
        alert('Flashcard atualizado!'); 
        deleteCachedAudio(editingId); // Limpa áudio
        setPodcastCache(prev => { const n = new Set(prev); n.delete(editingId); return n; }); // Remove tag visual
        cancelEdit();
      } else {
        const userId = (await (supabase.auth as any).getUser()).data.user?.id;
        if (!userId) throw new Error('Usuário não autenticado');
        const { error } = await supabase.from('flashcards').insert([{ user_id: userId, materia: newCard.materia, assunto: newCard.assunto, front: newCard.front, back: newCard.back, status: 'novo' as LocalFlashcard['status'] }]);
        if (error) throw error;
  ...
  const endSession = () => { setStudyQueue([]); setCurrentCardIndex(0); setIsFlipped(false); setAiStreamText(""); setFollowUpQuery(""); setShowSessionSummary(false); };
  ...
  // Efeito para carregar o conteúdo do card atual
  useEffect(() => {
      if (studyQueue.length > 0 && currentCardIndex < studyQueue.length) {
          ...
          // CARREGA EXPLICAÇÃO SALVA SE EXISTIR
          if (card.ai_explanation) {
              setAiStreamText(card.ai_explanation);
          } else {
              setAiStreamText("");
          }
      }
  ...
  const handlePlayNeural = async () => {
      ...
      // PREFERÊNCIA: Tenta usar o ID do áudio original (se for clone), senão o próprio ID
      const audioIdToUse = currentCard.original_audio_id || currentCard.id;

      const cancel = await handlePlayRevisionAudio(
          aiStreamText,
          audioIdToUse, 
          key,
          () => setIsPlayingNeural(true),
          () => setIsPlayingNeural(false),
          (err) => { alert(err); setIsPlayingNeural(false); }
      );
      setStopNeural(() => cancel);
  };

  const handlePodcastDuo = async () => {
      if (isPlayingNeural || isGeneratingPodcast) {
          if (stopNeural) stopNeural();
          setIsPlayingNeural(false);
      ...
      if (!aiStreamText || !currentCard) return; // Precisa do card para pegar o ID
      const key = getGeminiKey();
      ...
      const cancel = await generatePodcastAudio(
          aiStreamText,
          audioIdToUse,
          key,
  ...
  // Parar áudio ao desmontar ou mudar de card
  useEffect(() => {
      return () => { if (stopNeural) stopNeural(); setIsPlayingNeural(false); setIsGeneratingPodcast(false); };
  ...
  const handleSendFollowUp = async () => {
      ...
      // NOTA: Follow-up questions NÃO são salvas como explicação principal para não poluir o card base.
  ...
  useEffect(() => { 
      loadFlashcards(); 
      syncPodcastCache(); // Checa arquivos existentes
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 p-4 md:p-8">
      {/* ... (rest of the file remains similar until Manage Tab) ... */}
      <div className="max-w-7xl mx-auto">
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                ...
                <div className="mt-6">
                  ...
                  {/* ÁREA DA EXPLICAÇÃO E CHAT DE APROFUNDAMENTO */}
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 bg-slate-900/80 border border-white/10 rounded-2xl p-6 animate-in slide-in-from-bottom-4">
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
                              ...
                              {/* NOVO: Botões de Áudio (Neural + Podcast) */}
                              {aiStreamText && !aiLoading && (
                          ...
                          {aiStreamText && !aiLoading && (
                              <div className="pt-4 border-t border-white/10 animate-in fade-in slide-in-from-bottom-2">
        ...
        {activeTab === 'manage' && (
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4"><h3 className="text-xl font-bold flex items-center gap-2"><Layers className="text-cyan-400" /> Gerenciar Flashcards</h3><div className="flex flex-wrap gap-3"><button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-bold flex items-center gap-2"><Filter size={14} /> Filtros</button><CustomFilterDropdown label="Matéria" value={filterMateria} options={materias} onChange={setFilterMateria} icon={<BookOpen size={14} />} widthClass="w-48" />
            <div className="flex items-center gap-2">
                <CustomFilterDropdown label="Podcast" value={filterPodcast} options={['Todos', 'Com Podcast', 'Sem Podcast']} onChange={setFilterPodcast} icon={<Mic2 size={14} />} widthClass="w-full md:w-40" />
                <button onClick={syncPodcastCache} disabled={isSyncing} className="p-3 bg-slate-900/50 border border-white/10 rounded-xl hover:bg-white/5 transition-colors text-slate-400 hover:text-white" title="Sincronizar áudios">
                    <RefreshCw size={16} className={isSyncing ? "animate-spin text-cyan-400" : ""} />
