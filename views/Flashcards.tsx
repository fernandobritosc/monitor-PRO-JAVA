...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Shuffle, Eye, Sparkles, AlertTriangle, Settings, Volume2, Info, List, RefreshCw, Lock, ChevronLeft, ChevronRight
} from 'lucide-react';
                  ...
                  <div className={`absolute inset-0 backface-hidden bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-cyan-500/30 rounded-2xl p-5 md:p-8 flex flex-col ${isFlipped ? 'opacity-0' : 'opacity-100'}`}>
                    <div className="flex justify-between items-start mb-4 md:mb-6 shrink-0">
                        ...
                        <div className="flex gap-2 shrink-0">
                            ...
                            <span className="px-3 py-1 bg-slate-800 text-slate-400 rounded-full text-[10px] md:text-xs font-bold h-8 flex items-center">{currentCardIndex + 1}/{studyQueue.length}</span>
                        </div>
                    </div>
                    {/* SCROLLABLE CONTENT */}
