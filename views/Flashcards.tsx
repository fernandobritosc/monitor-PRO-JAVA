...
}> = ({ title, icon, content, isLoading, isMarkdown = false, children }) => {
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const handleGenerateExtraFormat = async (format: 'mapa' | 'fluxo' | 'tabela' | 'info') => {
      ...
      try {
          ...
          switch (format) {
              case 'mapa':
                  prompt = `Crie um mapa mental em formato de texto aninhado (usando traços) sobre o seguinte conceito: ${concept}. Comece com o conceito central e ramifique para os pontos-chave, detalhes e exemplos. Use formatação Markdown para ênfase (negrito).`;
                  break;
              case 'fluxo':
                  prompt = `Gere um fluxograma em texto descrevendo o processo ou sequência lógica de: ${concept}. Use setas (->) e caixas ( [Texto] ) para representar o fluxo. Use formatação Markdown para negrito. Se for um conceito, não um processo, explique por que um fluxograma não se aplica e sugira uma estrutura lógica.`;
                  break;
              case 'tabela':
                  prompt = `Crie uma tabela comparativa em formato Markdown. Se o conceito ${concept} envolver uma comparação, compare os dois. Se não, compare o conceito com um conceito relacionado ou oposto para destacar suas características únicas. A tabela deve ter pelo menos 2 colunas e 3 linhas de critérios.`;
                  break;
              case 'info':
                  prompt = `Gere um 'Infográfico Rápido' em formato de texto estruturado sobre: ${concept}. Use emojis como ícones, títulos curtos em MAIÚSCULAS, e bullet points (-) para os dados-chave. Use formatação Markdown (negrito). Organize a informação de forma visualmente clara e concisa.`;
                  break;
          }
                ...
                <div className="mt-6">
                  ...
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 animate-in slide-in-from-bottom-4 space-y-4">
                          <AIContentBox title={`Explicação (${getActiveProviderName()})`} icon={<Sparkles size={12} />} content={aiStreamText} isLoading={aiLoading}>
                                  ...
                                  {(['mapa', 'tabela', 'fluxo', 'info'] as const).map(format => {
                                    ...
                                    return (
                                      <button 
                                        ...
                                        disabled={extraLoading || aiLoading || !aiStreamText} 
                          ...
                          <AIContentBox title="Mnemônico Musical" icon={<Music size={12} />} content={mnemonicText} isLoading={mnemonicLoading} isMarkdown={false}>
