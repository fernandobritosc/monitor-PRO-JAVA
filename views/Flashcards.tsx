...
// FIX: Removed 'speakTextWithGemini' as it's not an exported member of aiService.
...
const MarkdownRenderer: React.FC<{ content: string }> = ({ content }) => {
  const parts = useMemo(() => {
    ...
    // Divide por tabelas para tratá-las separadamente
    const tableRegex = /(\|.*\|(?:\r?\n\|.*\|)+)/g;
    const splitByTables = content.split(tableRegex);

    return splitByTables.map((part, index) => {
      if (part.match(tableRegex)) { // É uma tabela
        ...
        return (
          <div key={index} className="overflow-x-auto my-4 bg-slate-950/30 rounded-lg border border-slate-700">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-700">
                  {header.map((h, i) => <th key={i} className="p-3 text-xs font-bold uppercase text-slate-400">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {body.map((row, i) => (
                  <tr key={i} className="border-b border-slate-700 last:border-0 hover:bg-white/5">
                    {row.map((cell, j) => <td key={j} className="p-3 text-sm text-slate-300 font-mono">{cell}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        return part.split('\n').map((line, lineIndex) => {
          if (line.trim().startsWith('- ') || line.trim().startsWith('* ')) {
            return <li key={`${index}-${lineIndex}`} className="my-1 ml-5 text-sm">{line.replace(/[-*]\s/, '')}</li>;
          }
          if (line.trim().startsWith('# ')) {
            return <h4 key={`${index}-${lineIndex}`} className="text-lg font-bold mt-4 mb-2">{line.replace(/#\s/, '')}</h4>;
          }
          return <p key={`${index}-${lineIndex}`} className="my-2 text-sm text-slate-300 whitespace-pre-wrap">{line}</p>;
        });
      }
    });
  }, [content]);
...
const AIContentBox: React.FC<{
  ...
  onRegenerate?: () => void;
}> = ({ title, icon, content, isLoading, isMarkdown = false, children, onRegenerate }) => {
  ...
  return (
    <div className="mt-4 bg-slate-900/80 border border-white/10 rounded-2xl p-6 animate-in slide-in-from-bottom-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-2">
           {onRegenerate && content && !isLoading && (
              <button onClick={onRegenerate} className="p-2 bg-slate-800/50 rounded-lg text-slate-400 hover:text-cyan-400 transition-all" title="Gerar novamente">
                <RefreshCw size={14} />
              </button>
            )}
           {content && (
             <button onClick={handleCopy} className="p-2 bg-slate-800/50 rounded-lg text-slate-400 hover:text-white transition-all">
      ...
      {isLoading ? (
        ...
        <div className="leading-relaxed max-h-[400px] overflow-y-auto custom-scrollbar pr-2 pb-2">
          {isMarkdown ? <MarkdownRenderer content={content} /> : <div className="text-slate-300 whitespace-pre-wrap text-sm">{content}</div>}
          {children}
        </div>
      )}
    </div>
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const saveAiAsset = async (assetType: keyof NonNullable<LocalFlashcard['ai_generated_assets']>, content: string) => {
      ...
      try {
          ...
          if (error) throw error;
          console.log(`✅ Asset '${assetType}' salvo para o card ${currentCard.id}`);
      } catch (error) {
          console.error("Erro ao salvar asset de IA:", error);
      }
  };

  const generateAIExplanation = async (forceRegenerate: boolean = false) => {
    if (!currentCard) return;

    if (!forceRegenerate && currentCard.ai_generated_assets?.explanation) {
        setAiStreamText(currentCard.ai_generated_assets.explanation);
        setAiLoading(false);
        return;
    }
    
    setAiLoading(true); setAiStreamText(''); setFollowUpQuery('');
    
    const preferred = selectedAI === 'auto' ? undefined : selectedAI;
    let accumulatedText = "";
    
    // Unifica o prompt para ser mais estruturado e de alta qualidade para ambos os provedores
    const prompt = `Atue como um especialista em concursos públicos. Sua resposta deve ser estruturada.
1.  **Explicação Direta:** Explique o conceito de forma concisa (máximo 3 linhas).
2.  **Exemplo Prático:** Forneça um exemplo claro de aplicação.
3.  **Macete Mnemônico:** Crie um mnemônico ou uma rima curta e eficaz para memorização.
...
`;
    ...
    await streamAIContent(prompt, { 
      onChunk: (text) => { setAiStreamText(prev => prev + text); accumulatedText += text; }, 
  ...
  const handleGenerateMnemonic = async (forceRegenerate: boolean = false) => {
    if (!currentCard || mnemonicLoading) return;

    if (!forceRegenerate && currentCard.ai_generated_assets?.mnemonic) {
      setMnemonicText(currentCard.ai_generated_assets.mnemonic);
      return;
    }
  ...
  const handleGenerateExtraFormat = async (format: 'mapa' | 'fluxo' | 'tabela' | 'info', forceRegenerate: boolean = false) => {
      if (!currentCard || extraLoading) return;
      setExtraFormat(format);
      
      if (!forceRegenerate && currentCard.ai_generated_assets?.[format]) {
          setExtraContent(currentCard.ai_generated_assets[format]!);
          return;
      }
      ...
      try {
          ...
          switch (format) {
              case 'mapa': prompt = `Crie um mapa mental em formato de texto aninhado (usando traços) sobre o seguinte conceito: ${concept}. Comece com o conceito central e ramifique para os pontos-chave, detalhes e exemplos. Use formatação Markdown para ênfase (negrito).`; break;
              case 'fluxo': prompt = `Gere um fluxograma em texto descrevendo o processo ou sequência lógica de: ${concept}. Use setas (->) e caixas ( [Texto] ) para representar o fluxo. Use formatação Markdown para negrito. Se for um conceito, não um processo, explique por que um fluxograma não se aplica e sugira uma estrutura lógica.`; break;
              case 'tabela': prompt = `Crie uma tabela comparativa em formato Markdown. Se o conceito ${concept} envolver uma comparação, compare os dois. Se não, compare o conceito com um conceito relacionado ou oposto para destacar suas características únicas. A tabela deve ter pelo menos 2 colunas e 3 linhas de critérios.`; break;
              case 'info': prompt = `Gere um 'Infográfico Rápido' em formato de texto estruturado sobre: ${concept}. Use emojis como ícones, títulos curtos em MAIÚSCULAS, e bullet points (-) para os dados-chave. Use formatação Markdown (negrito). Organize a informação de forma visualmente clara e concisa.`; break;
          }
  ...
  const handleSendFollowUp = async () => {
      ...
      await streamAIContent(contextPrompt, { 
        ...
        onError: (error) => { setAiStreamText(prev => prev + '\n[Erro na resposta]'); setAiLoading(false); } 
      }, getGeminiKey(), getGroqKey(), preferred);
  };
          ...
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                ...
                <div className="mt-6">
                  <button onClick={() => generateAIExplanation()} disabled={aiLoading} className="w-full px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-purple-500/20 active:scale-95"><Sparkles size={18} />{aiLoading ? 'Gerando...' : (aiStreamText ? 'Explicação Gerada' : 'Pedir Explicação à IA')}</button>
                  
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 animate-in slide-in-from-bottom-4 space-y-4">
                          <AIContentBox 
                            title={`Explicação (${getActiveProviderName()})`} 
                            icon={<Sparkles size={12} />} 
                            content={aiStreamText} 
                            isLoading={aiLoading}
                            onRegenerate={() => generateAIExplanation(true)}
                          >
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full mt-4">
                                <div>
                                  <button onClick={handlePlayNeural} disabled={isGeneratingPodcast} className={`w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${isPlayingNeural && !isGeneratingPodcast ? 'bg-red-500/20 text-red-400 border-red-500/50' : 'bg-slate-800 text-slate-300 border-white/10 hover:bg-slate-700'}`}>{isPlayingNeural && !isGeneratingPodcast ? <><Square size={10} className="fill-current" /> Parar</> : <><Headphones size={12} /> Ouvir (Voz Neural)</>}</button>
                                  <p className="text-[10px] text-slate-500 mt-1.5 px-1 text-center sm:text-left">Ouve a explicação com uma voz de alta qualidade.</p>
                                </div>
                                <div>
                                  <button onClick={handlePodcastDuo} disabled={isPlayingNeural && !isGeneratingPodcast} className={`w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${isGeneratingPodcast ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50' : 'bg-gradient-to-r from-cyan-600/80 to-blue-600/80 text-white border-transparent hover:opacity-90'}`}>{isGeneratingPodcast ? <><Square size={10} className="fill-current" /> {podcastStatus || "Gerando..."}</> : <><Mic2 size={12} /> Podcast (Duo)</>}</button>
                                  <p className="text-[10px] text-slate-500 mt-1.5 px-1 text-center sm:text-left">Gera um diálogo em áudio entre dois apresentadores sobre o tema.</p>
                                </div>
                              </div>
                          </AIContentBox>
                          
                          <div className="pt-4 border-t border-white/10">
                              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Ferramentas de Aprofundamento</h4>
                              <p className="text-[10px] text-slate-500 mb-3">Visualize a informação de novas maneiras para reforçar o aprendizado.</p>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                  {(['mapa', 'tabela', 'fluxo', 'info'] as const).map(format => {
                                    const config = {
                                      mapa: { icon: MapIcon, label: 'Mapa Mental', description: 'Estrutura em tópicos e subtópicos.' },
                                      fluxo: { icon: ArrowRightLeft, label: 'Fluxograma', description: 'Mostra a sequência de um processo.' },
                                      tabela: { icon: Table, label: 'Tabela Comp.', description: 'Compara com um tema oposto.' },
                                      info: { icon: FileText, label: 'Infográfico', description: 'Resume com ícones e pontos-chave.' }
                                    }[format];
                                    return (
                                      <button 
                                        key={format} 
                                        onClick={() => handleGenerateExtraFormat(format, extraFormat === format)} 
                                        disabled={extraLoading || aiLoading} 
                                        className={`flex flex-col items-center justify-start text-center gap-2 p-3 h-full rounded-xl text-xs transition-all border disabled:opacity-50 ${extraFormat === format ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30' : 'bg-slate-800 text-slate-300 border-white/10 hover:bg-slate-700'}`}
                                      >
                                        <config.icon size={18} />
                                        <span className="font-bold text-white">{config.label}</span>
                                        <span className="text-[10px] text-slate-400 font-normal leading-tight">{config.description}</span>
                                      </button>
                                    );
                                  })}
                              </div>
                          ...
                          <AIContentBox 
                            ...
                            onRegenerate={() => handleGenerateMnemonic(true)}
                          >
                            <p className="text-xs text-slate-400 mb-3 -mt-1">Cria uma rima ou música curta para ajudar na memorização do conteúdo do flashcard.</p>
                            {!mnemonicText && !mnemonicLoading && (
                              <button onClick={() => handleGenerateMnemonic()} disabled={mnemonicLoading || aiLoading || !aiStreamText} className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all bg-purple-500/10 text-purple-300 hover:bg-purple-500/20 disabled:opacity-50">{mnemonicLoading ? <Loader2 size={14} className="animate-spin" /> : <Music size={14} />}{mnemonicLoading ? "Criando melodia..." : "Gerar Mnemônico"}</button>
                            )}
                          </AIContentBox>

                          {extraFormat && <AIContentBox title={extraFormat} icon={<Zap size={12} />} content={extraContent} isLoading={extraLoading} isMarkdown={true} />}
                          ...
                          <div className="pt-4 mt-2 border-t border-white/10">
                            ...
                            <div className="flex gap-2"><input type="text" className="flex-1 bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all placeholder-slate-600" placeholder="Ex: Pode dar mais um exemplo?" value={followUpQuery} onChange={(e) => setFollowUpQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendFollowUp()} /><button onClick={handleSendFollowUp} disabled={!followUpQuery.trim()} className="bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed text-white p-3 rounded-xl transition-all shadow-lg shadow-purple-500/10"><Send size={18} /></button></div>
                          </div>
                      </div>
                  )}
                </div>
