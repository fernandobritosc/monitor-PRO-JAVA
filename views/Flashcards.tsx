...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Shuffle, Eye, Sparkles, AlertTriangle, Settings, Volume2, Info, List
} from 'lucide-react';
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const handleCardResult = async (status: LocalFlashcard['status']) => {
    ...
    // 1. Atualiza no banco (background)
    updateCardStatus(currentCard.id, status);

    // 2. Navegação Otimista
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                ...
                <div className="flex justify-center gap-4 mt-8">
                  ...
                  <div className="flex gap-2">
                    {/* BOTÕES CORRIGIDOS COM handleCardResult */}
        ...
        {activeTab === 'manage' && (
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            <div className="bg-slate-900/30 border border-white/5 rounded-2xl p-6">
                <h4 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Plus size={18} /> Criar Novo Flashcard</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    {/* CAMPO ASSUNTO RESTAURADO */}
                    <div className="md:col-span-2">
                        <label className="block text-sm font-bold text-slate-400 mb-2">Assunto (Opcional)</label>
                        <input type="text" value={newCard.assunto} onChange={(e) => setNewCard({...newCard, assunto: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 outline-none" placeholder="Ex: Crase, Atos Administrativos..." />
                    </div>
                </div>
                <div className="mb-4">
                ...
                <button onClick={saveCard} className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-bold flex items-center gap-2"><Save size={16} /> Salvar</button>
            </div>
