                      ...
                      <div className="mt-4 animate-in slide-in-from-bottom-4 space-y-4">
                          <AIContentBox title={`Explicação (${getActiveProviderName()})`} icon={<Sparkles size={12} />} content={aiStreamText} isLoading={aiLoading}>
                              <div className="flex flex-wrap gap-2 w-full mt-4">
                                  <button onClick={handlePlayNeural} disabled={isGeneratingPodcast} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${isPlayingNeural && !isGeneratingPodcast ? 'bg-red-500/20 text-red-400 border-red-500/50' : 'bg-slate-800 text-slate-300 border-white/10 hover:bg-slate-700'}`}>{isPlayingNeural && !isGeneratingPodcast ? <><Square size={10} className="fill-current" /> Parar</> : <><Headphones size={12} /> Ouvir (Voz)</>}</button>
                                  <button onClick={handlePodcastDuo} disabled={isPlayingNeural && !isGeneratingPodcast} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${isGeneratingPodcast ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50' : 'bg-gradient-to-r from-cyan-600/80 to-blue-600/80 text-white border-transparent hover:opacity-90'}`}>{isGeneratingPodcast ? <><Square size={10} className="fill-current" /> {podcastStatus || "Gerando..."}</> : <><Mic2 size={12} /> Podcast (Duo)</>}</button>
                              </div>
                          </AIContentBox>
                          
                          <div className="pt-4 border-t border-white/10">
                              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Ferramentas de Geração</h4>
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                  {(['mapa', 'tabela', 'fluxo', 'info'] as const).map(format => {
                                    const config = {
                                      mapa: { icon: MapIcon, label: 'Mapa Mental' },
                                      fluxo: { icon: ArrowRightLeft, label: 'Fluxograma' },
                                      tabela: { icon: Table, label: 'Tabela Comp.' },
                                      info: { icon: FileText, label: 'Infográfico Rápido' }
                                    }[format];
                                    return (
                                      <button key={format} onClick={() => handleGenerateExtraFormat(format)} disabled={extraLoading || aiLoading || !aiStreamText} className={`flex flex-col items-center justify-center gap-1.5 p-3 rounded-lg text-xs font-bold transition-all border disabled:opacity-50 ${extraFormat === format ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30' : 'bg-slate-800 text-slate-300 border-white/10 hover:bg-slate-700'}`}><config.icon size={14} /><span>{config.label}</span></button>
                                    );
                                  })}
                              </div>
                          </div>
