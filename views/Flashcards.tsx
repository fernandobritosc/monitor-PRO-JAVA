...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Eye, Sparkles, AlertTriangle, Volume2, Info, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus, ChevronUp, Headphones, Square, Mic2, FileAudio, RefreshCw, User
} from 'lucide-react';
...
-- IMPORTANTE: Permite leitura pública para a aba Comunidade funcionar, 
-- MAS o frontend DEVE filtrar por user_id nas abas privadas.
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const loadFlashcards = async () => {
    ...
    try {
      ...
      // FIX: Filtra EXPLICITAMENTE pelo user_id para não mostrar cards de outros usuários
  ...
  const loadCommunityDecks = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('flashcards')
        ...
        // Opcional: .neq('user_id', user?.id) se quiser esconder os próprios cards da comunidade

      if (error) throw error;
      const decksMap = new Map();
      data?.forEach((card: any) => {
        // Evita mostrar cards que eu já tenho (opcional, aqui estamos permitindo ver tudo)
  ...
  const importCards = async (cardsToImport: any[], type: 'deck' | 'topic' | 'single', topicName?: string) => {
      ...
      try {
          ...
          const payload = cardsToImport.map(c => ({ 
              ...
              // Ao clonar, você se torna o "dono" para edição, mas podemos manter um rastro se quiséssemos. 
              // Por enquanto, a assinatura do novo card será a do usuário atual.
    ...
    try {
      if (editingId) {
        ...
        const { error } = await supabase.from('flashcards').insert([{ 
            ...
            front: newCard.front, 
            back: newCard.back, 
            author_name: authorName, // Assinatura do Autor
            status: 'novo' as LocalFlashcard['status'] 
        }]);
        if (error) throw error;
