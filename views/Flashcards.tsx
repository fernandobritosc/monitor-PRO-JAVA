...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Eye, Sparkles, AlertTriangle, Volume2, Info, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus, ChevronUp, Headphones, Square, Mic2, FileAudio
} from 'lucide-react';
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const syncPodcastCache = async () => {
      try {
          const { data } = await supabase.storage.from('audio-revisions').list('', { limit: 1000, search: '_podcast.wav' });
          if (data) {
              // O nome do arquivo é ID_podcast.wav
              const ids = new Set<string>(data.map((f: any) => f.name.replace('_podcast.wav', '')));
              setPodcastCache(ids);
          }
      } catch (e) {
          console.error("Erro ao sincronizar cache de podcast:", e);
      }
  };
