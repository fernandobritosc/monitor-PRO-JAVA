...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  useEffect(() => {
      // Limpa os campos de IA quando o provedor de IA é alterado
      setAiStreamText("");
      setMnemonicText("");
      setExtraContent('');
      setExtraFormat(null);
          ...
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                ...
                <div className="mt-6">
                  <button onClick={generateAIExplanation} disabled={aiLoading} className="w-full px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-purple-500/20 active:scale-95"><Sparkles size={18} />{aiLoading ? 'Gerando...' : (aiStreamText ? 'Gerar Novamente' : 'Pedir Explicação à IA')}</button>
                  ...
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 animate-in slide-in-from-bottom-4 space-y-4">
                          ...
                          {extraFormat && <AIContentBox title={extraFormat} icon={<Zap size={12} />} content={extraContent} isLoading={extraLoading} isMarkdown={true} onRegenerate={() => handleGenerateExtraFormat(extraFormat)} />}
