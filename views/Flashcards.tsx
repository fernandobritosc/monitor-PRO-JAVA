...
import { 
  ...
  Globe, Database, Copy, ChevronDown, Eye, Sparkles, AlertTriangle, Volume2, Info, Lock, ChevronLeft, ChevronRight, Trophy, Target, Tag, Send, MessageSquarePlus
} from 'lucide-react';
...
// SCRIPT SQL CORRIGIDO: Adiciona DROP POLICY antes de CREATE POLICY
...
-- POLÍTICAS DE ACESSO (REMOVE ANTES DE CRIAR PARA EVITAR ERRO 42710)
...
-- ÍNDICE ÚNICO (REMOVE ANTES DE CRIAR)
...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const importCards = async (cardsToImport: any[], type: 'deck' | 'topic' | 'single', topicName?: string) => {
      ...
      try {
          ...
          const payload = cardsToImport.map(c => ({ user_id: userId, materia: c.materia, assunto: c.assunto, front: c.front, back: c.back, status: 'novo' as LocalFlashcard['status'] }));
  ...
  const saveOrUpdateCard = async () => {
    ...
    try {
      if (editingId) {
        const { error } = await supabase.from('flashcards').update({ materia: newCard.materia, assunto: newCard.assunto, front: newCard.front, back: newCard.back }).eq('id', editingId);
        if (error) throw error; alert('Flashcard atualizado!'); cancelEdit();
      } else {
        const userId = (await (supabase.auth as any).getUser()).data.user?.id;
        if (!userId) throw new Error('Usuário não autenticado');
  ...
  const deleteCard = async (id: string) => { if (!confirm('Excluir este flashcard?')) return; try { const { error } = await supabase.from('flashcards').delete().eq('id', id); if (error) throw error; loadFlashcards(); } catch (error) { console.error(error); } };
  ...
  const handleCardResult = async (status: LocalFlashcard['status']) => {
    ...
    if (nextIndex < studyQueue.length) { setIsFlipped(false); setAiStreamText(""); setFollowUpQuery(""); setCurrentCardIndex(nextIndex); } 
    else { setShowSessionSummary(true); }
  };
                ...
                <div className="mt-6">
                  ...
                  {(aiStreamText || aiLoading) && (
                      <div className="mt-4 bg-slate-900/80 border border-white/10 rounded-2xl p-6 animate-in slide-in-from-bottom-4">
                          <div className="flex items-center gap-2 mb-4 text-purple-400 font-bold text-xs uppercase tracking-widest"><Sparkles size={12} /> Explicação ({getActiveProviderName()})</div>
          ...
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            <div className="bg-slate-900/30 border border-white/5 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div className="md:col-span-2"><label className="block text-sm font-bold text-slate-400 mb-2 flex items-center gap-2">Assunto (Opcional){!editingId && newCard.assunto && <span className="text-[10px] text-green-400 bg-green-500/10 px-1.5 py-0.5 rounded flex items-center gap-1"><Lock size={8} /> Fixado</span>}</label><input type="text" list="flashcard-topics" value={newCard.assunto} onChange={(e) => setNewCard({...newCard, assunto: e.target.value})} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 outline-none" placeholder="Ex: Crase, Atos Administrativos..." />{newCard.materia && (<datalist id="flashcard-topics">{availableTopics.map(t => <option key={t} value={t} />)}</datalist>)}</div>
                </div>
