...
const Flashcards: React.FC<FlashcardsProps> = ({ missaoAtiva, editais }) => {
  ...
  const hasExplanation = currentCard?.ai_generated_assets?.explanation;
  const hasMnemonic = currentCard?.ai_generated_assets?.mnemonic;
          ...
          <div className="glass rounded-2xl p-6 shadow-xl space-y-6">
            ...
            {studyQueue.length === 0 ? (
              ...
              <div className="perspective-1000">
                ...
                <div className="mt-6">
                  <button onClick={() => generateAIExplanation()} disabled={aiLoading} className="w-full px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-purple-500/20 active:scale-95"><Sparkles size={18} />{aiLoading ? 'Gerando...' : (hasExplanation && !aiStreamText) ? 'Ver Explicação Gerada' : (aiStreamText ? 'Explicação Gerada' : 'Pedir Explicação à IA')}</button>
